...
def bank_deposit(amount: float, term: int, replenishment: float) -> float:
    def calc_replenishment(deposit_rate) -> float:
        return replenishment * (1 + deposit_rate / 100)

    rate = get_rate(amount, term)
    return amount * (1 + rate / 100 * term / ONE_YEAR) + calc_replenishment(rate) * (term - 2)


if __name__ == '__main__':
    investment = bank_deposit(100500, 12, 1000)
    print(investment)
